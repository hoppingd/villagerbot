                ...
                // if user has room in storage
                    ...
                    if (reactorData.blaTier == constants.UPGRADE_COSTS.length) {
                        ...
                        if (randIdx < constants.BLATHERS_BONUS_CHANCE && rarity < constants.RARITY_NAMES.length - 1) { // if the odds hit and the card isn't max rarity
                            rarity += 1;
                            try { interaction.channel.send(`${villager.name} rarity upgraded to ${constants.RARITY_NAMES[rarity]} by <:blathers:1349263646206857236> **Blathers V**.`); } catch (APIError) { console.log("Could not send follow up message. The channel may have been deleted."); }
                        }
                    }
                    // BLATHERS III BONUS
